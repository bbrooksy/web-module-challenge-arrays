...
function is31Flavors(checkArr){
    for(let i =0; i <originalFlavors.length;
       i++)
      if(checkArr.length===31){
          return true;
      } else {
          return false;
      }
...
function addFlavor(originalFlavors){
   let bestFlav = originalFlavors
   bestFlav.unshift("Rainbow Sherbert");
    return bestFlav;
}
...
function removeFlavorByName(originalFlavors, remFlav){
    let bestFlav = originalFlavors;
    bestFlav.splice(originalFlavors.indexOf(remFlav),1);
    return bestFlav
}
...
function filterByWord(originalFlavors, flavName){
    let bestFlav = [];

    for(let i =0; i< originalFlavors.length; i++){
        if (originalFlavors[i].includes(flavName)){
            bestFlav.push(originalFlavors[i]);
        }
    }
    return bestFlav;
}
